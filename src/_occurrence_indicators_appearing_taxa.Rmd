...
app_reapp_global <- 
  map2(appearing_reappearing_taxa_prot_areas,
       ...
       function(x,y) {
         full_join(
           x %>% 
             mutate(natura2000 = TRUE),
           y, by = names(x)) %>%
           mutate(natura2000 = ifelse(is.na(natura2000), FALSE, TRUE))
    }
)
...
Save taxa from unified checklist without linked occurrences:

...
```{r read_preprocessed_data, cache = TRUE, message = FALSE}
df_ts <- read_tsv(
...
We define the function `is_appearing` for appearing/reappearing detection (this function will be moved to TrIAS package `trias` if this workflow will be positively reviewed):
...
First, we read data used as input for checklist indicators, i.e. the output of [checklist indicators preprocessing pipeline](https://trias-project.github.io/indicators/get_data_input_checklist_indicators.html):

```{r read_data_input_checklist_inds}
checklist_df<- read_tsv(
  here::here("data", "interim", "data_input_checklist_indicators.tsv"),
  na = "",
  guess_max = 5000
)
```
...
Taxa defined as `PRESENT` in unified checklist but still without occurrences:
...
write_tsv(taxa_from_checklist_df,
          path = here::here("data", 
                            "output", 
                            "present_taxa_without_occurrences.tsv"),
          na = ""
)
```

...
Contrarily to distribution information, description contains a column  `type` with the following descriptions: 

```{r}
description %>% distinct(type)
```
Values related to such descriptors are in column  `description`. 
We would like to spread this information haveing different descriptors as different columns:

```{r}
description %<>%
   group_by(type, taxonKey) %>%
   mutate(ind = row_number()) %>%
   spread(key = type, value = description) %>% 
   select(-ind) %>% janitor::clean_names()
```
...
1. invasion stage (`establishmentMeans` equal to one of `INTRODUCED`, NATURALISED", "INVASIVE",
                                   "ASSISTED COLONISATION` )
2. presence status (`status` not equal to `ABSENT`, `EXCLUDED` or `EXTINCT`)
...
merged_info %<>%
  filter(establishmentMeans %in% c("INTRODUCED", "NATURALISED", "INVASIVE",
                                   "ASSISTED COLONISATION"),
         !status %in% c("ABSENT", "EXCLUDED", "EXTINCT"),
         country == "BE")
```
...
merged_info %<>% separate(col = temporal,
                          ...
                          fill = "right",
                          remove = TRUE) %>%
  mutate(endDate = ifelse(is.na(endDate),
                          startDate,
                          endDate))
```
...
```{r split_pathway}
merged_info %<>% rowwise() %>%
  mutate(pathway_level1 = ifelse(type == "pathway",
                                 str_split_fixed(str_split(description,
                                                           pattern = "pathway:")[[1]][2],
                                                 pattern = "_", n = 2)[[1]][1],
                                 NA),
         pathway_level2 = ifelse(type == "pathway",
                                 str_split_fixed(str_split(description,
                                                           pattern = "pathway:")[[1]][2],
                                                 pattern = "_", n = 2)[[2]][1],
                                 NA))

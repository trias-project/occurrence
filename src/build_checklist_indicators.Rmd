...
description_tidy <- 
  ...
  purrr::map2(types, ~ change_colname(.x, .y)) %>%
  purrr::reduce(dplyr::full_join, by = "taxonKey")
head(description_tidy)
```

The chunk code above should be wrapped in a function: Peter Desmet proposed the name `spread_with_duplicates`. It could be put in project package, `trias`, or be part of `dplyr` package eventually. See GitHub issue [#]
...
```{r pathway_levels_example, echo = FASLE}

...
em_status_dr <- function(df, 
  ...
  # # # dr_7: Increase? Last value > before last value > 0
  # dr_7 <-
  #   df %>%
  #   filter(year == max(year)) %>%
  #   select(taxonKey, occ) %>%
  ...
  # # dr_9: Decrease? Last value < before last value
  # dr_9 <-
  #   df %>%
  #   filter(year == max(year)) %>%
  #   select(taxonKey, occ) %>%
  ...
  #       rename(before_last_occ = occ),
  #     by = "taxonKey") %>%
  #   mutate(dr_9 = last_occ < before_last_occ) %>%
  #   select(taxonKey, dr_9)
...
em_decision_rules_occs_pa <- map_dfr(
  evaluation_years,
  ~ em_status_dr(df = df_ts_compact_prot_areas,
                 evaluation_year = .,
                 data_col = "obs")
)
```
...
map(gam_occupancy_pa[1:10], ~.$plot)
```

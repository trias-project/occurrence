...
Preview GAM ouput for modelling number of occurrences in Belgium:

```{r preview_gam_occs_BE}
em_gam_list$gam_occurrences_be %>% head(10)
```
...
To rank taxa equally placed, we use the average of the minimal guaranteed growth of number of occurrences in all Belgium over all the evaluation years. A preview of these values follows:

```{r add_mean_growth_occs_BE}
# add mean growth to em_df
em_df <-
  em_df %>%
  left_join(em_gam_list$gam_occurrences_be %>%
    group_by(taxonKey) %>%
...
We can also rank using a point strategy reflecting the same ranking strategy as before. We apply gain factors to all emerging status scores using the number of occurrences in 2016 in all Belgium as reference (gain factor  = 1). The following gain factors are applied:

| indicator | year | region | gain factor |
| --- | --- | --- | --- | 
occupancy | 2018 | protected areas | 3
occurrences | 2018 | protected areas | 2.5
occupancy | 2018 | Belgium | 2.5
occurrences | 2018 | Belgium | 2
occupancy | 2017 | protected areas | 2.5
occurrences | 2017 | protected areas | 2
occupancy | 2017 | Belgium | 2
occurrences | 2017 | Belgium | 1.5
occupancy | 2016 | protected areas | 2
occurrences | 2016 | protected areas | 1.5
occupancy | 2016 | Belgium | 1.5
occurrences | 2016 | Belgium | 1
...
ranking_pts <-
  ...
  rowwise() %>%
  mutate(em_pts = sum(
    year_2018_em_status_occupancy_pa * 3,
    year_2018_em_status_occurrences_pa * 2.5,
    year_2018_em_status_occupancy_BE * 2.5,
    year_2018_em_status_occurrences_BE * 2,
    year_2017_em_status_occupancy_pa * 2.5,

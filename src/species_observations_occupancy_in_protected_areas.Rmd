...
infraspecific_alien_status_species <-
  read_tsv(here::here("reference",                      "assessment_alien_status_species_with_alien_infraspecific_taxa.tsv"),
            na = "")
infraspecific_alien_status_species
```

where `key` is the key of the taxon as published in the unified checklist,  `nubKey` is the key of the corresponding taxon in the GBIF Backbone. Also the columns `speciesKey`, `species`, `scientificName` and  `kingdom` The latter provides also the information other 
...
occs_prot_areas <-
  occs_prot_areas %>%
  mutate(is_alien = speciesKey %in% be_alientaxa_info$taxonKey |
           speciesKey %in% infraspecific_alien_and_species$speciesKey)
```
...
To better study the spread of alien taxa we also add information about infraspecific taxa:

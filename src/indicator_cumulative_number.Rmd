...
indicator_total_year <- function(data, start_year_plot = 1940, 
                                 x_scale_stepsize = 10, facet_column = NULL) {
    
    # initial input checks
    assert_that(is.data.frame(data))
    valid_facet_options <- c("family", "order", "class", "phylum", "kingdom",
                             "pathway_level1", "locality", "native_range")
    facet_column <- match.arg(facet_column, valid_facet_options)    
    
    unique_df <- data %>%
        filter(!is.na(startDate)) %>%
        distinct(key, startDate, endDate, kingdom)

    # Create yearslist using startDate and endDate 
    counts_ias <- data.frame(year = c(min(unique_df$startDate, 
                                          na.rm = TRUE):max(unique_df$endDate, 
                                                            na.rm = TRUE))) %>%
        as_tibble()

    counts_ias <- counts_ias %>%
        rowwise() %>%
        do_(interp(~ count_presence(year = .$year, df = unique_df))) %>%
        bind_cols(counts_ias) %>% 
        select(year, everything()) %>%
        mutate(decade = year - year %% 10)
    
    maxDate <- max(counts_ias$year)
    
    top_graph <- ggplot(counts_ias, aes(x = year, y = counts)) +
        geom_line() +
        xlab("Year") +
        ylab("Number of alien species") +
        scale_x_continuous(breaks = seq(start_year_plot, maxDate, 
                                        x_scale_stepsize)) +
        xlim(start_year_plot, maxDate) +
        theme_inbo()
    ...
    if (is.null(facet_column)) {
      ...
      wrap_graph <- top_graph + 
        facet_wrap(facet_column)
      combined_graph <- multiplot(top_graph, wrap_graph)
...
```{r set_graph_parameters}
start_year_plot <- 1940
x_scale_stepsize <- 10

indicator_total_year(data, start_year_plot, x_scale_stepsize, facet_column = "kingdom")
```

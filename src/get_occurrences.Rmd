...
devtools::install_github("trias-project/trias",
                         ref = "remove_country_from_tsv") # remove it after merging to master
...
```{r define_taxa}
taxa_input_file <- "https://raw.githubusercontent.com/trias-project/pipeline/642fc791c56750e3a7b25b0ba9efa9d30d5736b4/data/input/modelling_species.tsv"
```
...
Tt can be useful to import occurrence data directly in R. 
You can do it by GBIF download key, which you can find in the column `gbif_download_key` (1st column of `gbif_downloads.tsv`). For example:

```{r import_ocurrence_from_gbif_key, evaluate = FALSE}
gbif_key <- "0002455-171124123535762"
file_path <- "../data/output/"
occ <- occ_download_get(key = gbifkey, overwrite = TRUE, path = file_path)
...
file.rename(file_to_extract,
            file_name)
occurrence <- read_delim(file_name, 

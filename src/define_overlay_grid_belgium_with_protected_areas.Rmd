...
```{r be_regions, cache = TRUE}
data(BE_ADMIN_REGION)
BE_ADMIN_REGION <- st_as_sf(BE_ADMIN_REGION)

regions_palette <- colorFactor(topo.colors(3), BE_ADMIN_REGION$CD_RGN_REFNIS)
leaflet(BE_ADMIN_REGION) %>%
  addTiles() %>%
  addPolygons(fillColor = ~regions_palette(CD_RGN_REFNIS),
              color = "black",
              weight = 1,
              opacity = 0.3,
              label = ~paste(TX_RGN_DESCR_NL,TX_RGN_DESCR_FR, ", "))
```
...
We show the protected areas for each of the three regions separately.

### Flemish protected areas

Protected areas of Flemish region:

```{r leaflet_prot_areas_flanders, hide = TRUE, cache = TRUE}
# Transform to wgs84
...
leaflet(protected_areas_wgs84 %>%
          filter(flanders == TRUE)) %>%
  addTiles() %>%
  addPolygons(fillColor = ~prot_area_fl_palette(SITECODE),
              fillOpacity = 0.7,
              color = "black",
              weight = 0.5,
              opacity = 1,
...
Protected areas of Wallonia region:

```{r leaflet_prot_areas_wa, hide = TRUE, cache = TRUE}
prot_area_wa_palette <- colorFactor(
  topo.colors(nrow(protected_areas_wallonia)),
  protected_areas_wallonia$SITECODE)
leaflet(protected_areas_wgs84 %>%
          filter(wallonia == TRUE)) %>%
  addTiles() %>%
  addPolygons(fillColor = ~prot_area_wa_palette(SITECODE),
              fillOpacity = 0.7,
              color = "black",
              weight = 0.5,
              opacity = 1,
...
### Brussels protected aras

Protected areas of Brussels region:

```{r leaflet_prot_areas_br, hide = TRUE, cache = TRUE}
prot_area_br_palette <- colorFactor(
  topo.colors(nrow(protected_areas_brussels)),
  protected_areas_brussels$SITECODE)
leaflet(protected_areas_wgs84 %>%
          filter(brussels == TRUE)) %>%
  addTiles() %>%
  addPolygons(fillColor = ~prot_area_br_palette(SITECODE),
              fillOpacity = 0.7,
              color = "black",
              weight = 0.5,
              opacity = 1,
...
### Protected aras under federal administration

Protected areas under federal administration:

```{r leaflet_prot_areas_fed_admin, hide = TRUE, cache = TRUE} 
prot_area_fed_palette <- colorFactor(
...
leaflet(protected_areas_wgs84 %>%
          filter(flanders == FALSE,
                 ...
                 brussels == FALSE)) %>%
  addTiles() %>%
  addPolygons(fillColor = ~prot_area_fed_palette(SITECODE),
              fillOpacity = 0.7,
              color = "black",
              weight = 0.5,
              opacity = 1,
...
```{r leaflet_SPA, hide = TRUE, cache = TRUE}
# Transform to WGS84
protected_areas_bird_wgs84 <-
  protected_areas_bird %>%
  st_transform(crs = 4326)

prot_area_type_palette <- colorFactor(
  topo.colors(2),
  protected_areas_bird_wgs84$SITETYPE)
leaflet(protected_areas_bird_wgs84) %>%
  addTiles() %>%
  addPolygons(fillColor = ~prot_area_type_palette(SITETYPE),
              fillOpacity = 0.7,
              color = "black",
              weight = 0.5,
              opacity = 1,
              label = ~SITENAME,
              popup = ~SITECODE) %>%
  addLegend(pal = prot_area_type_palette,
            values = ~SITETYPE,
            title = "SITETYPE",
            opacity = 1)
```
...
Map of habitat directive areas:

```{r leaflet_SAC, hide = TRUE, cache = TRUE}
# Transform to WGS84
protected_areas_habitat_wgs84 <-
  protected_areas_habitat %>%
  st_transform(crs = 4326)

prot_area_type_palette <- colorFactor(
  topo.colors(2),
  protected_areas_habitat_wgs84$SITETYPE)
leaflet(protected_areas_habitat_wgs84) %>%
  addTiles() %>%
  addPolygons(fillColor = ~prot_area_type_palette(SITETYPE),
              fillOpacity = 0.7,
              color = "black",

...
pres_abs_table_mov_sum <- pres_abs_table %>%
  ...
  mutate(mov_sum = c(rep(0, t_window - 1), 
...
data_mov_sum_tidy <- crossing(
  data.frame(scientificName = unique(data_mov_sum$scientificName),
             stringsAsFactors = FALSE),
  data.frame(year = seq(min(data_mov_sum$year, na.rm = TRUE),
...
ggplot(data_mov_sum_tidy,
       aes(x = year, y = mov_sum)) + ylab("n. records") +

...
Select downscale models:

```{r models}
models = c("Nachman", "PL", "Poisson", "Logis", "NB")
```

and cell areas:

```{r cell_areas}
# 1 by 1, 2 by 2, 3 by 3, 4 by 4 km2
...
downscaling_tidy <- map(
  names(downscaling), function(x) {
    downscaling[[x]]$Occupancy %>%
      mutate(scientificName = str_split(x, pattern = "_")[[1]][1],
...
downscaling_tidy <- crossing(
  data.frame(scientificName = species, 
             stringsAsFactors = FALSE),
  year = seq(min(downscaling_tidy$year, na.rm = TRUE),

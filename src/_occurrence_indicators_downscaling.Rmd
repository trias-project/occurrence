...
```{r species_left}
sc_names <- occ_df %>% distinct(scientificName) %>% pull() 
sc_names
```
...
We intersect occurrence circles with the grid per each species, grid and year:
...
pres_abs_table <- belgium_5grid %>% 
  ...
  crossing(data.frame(scientificName = sc_names, 
...
It would not have sense to apply downscaling to species occurring in less than 10 percentage of the squares.
...
We are interested on how occupancy varies with time. We need to apply downscaling modelling on data within time windows.
...
The temporal variability is very high due to changes in survey efforts. Such variability risks to covers variations in aliean species spreading. So, for each species and square, we aggregate data within a moving time window (here a window of 5 years is used. Note: a longer time window would be not policy relevant):
...
pres_abs_table_mov_sum <- pres_abs_table %>%
  ...
  mutate(mov_sum = ifelse(year < (max(year) - t_window + 1),
                          rollsum(na.fill(n, 0), 5, align = "left"),
                          0)) %>%
  ungroup() %>%
  mutate(presence = if_else(mov_sum > 0, 1, 0))
```

A moving sum has a second benefit: downscaling tends to underestimate occupancy if less than 2.5% percentage of squares are occupied. The moving sum could increase the number of occupied squares. We restrict our modelling to years and species when number of occupied squares are above the threshold:
...
First upgraining, then downscaling. See documentation package [`downscale`](https://cran.r-project.org/web/packages/downscale/downscale.pdf).
...
In case of small extent (Belgium is relatively small) this discrepancy can easily become very large. So, no further upgraining is applied. Here below we show the effects of choosing different thresholds:
...
upgrain_species_year <- map2(species_year$scientificName,
                             species_year$year,
                             function(x,y) {
...
names(upgrain_species_year) <- str_c(species_year$scientificName,
                                     species_year$year, sep = "_")
```

Run downscale models (simple ensemble as in article of Quentin Groom except INB which doesn't converge):

```{r apply_downscaling_models}
areas <- c(10^6, 4*10^6, 9*10^6, 16*10^6)
progress_bar <- progress_estimated(length(upgrain_species_year))
downscaling <- map(upgrain_species_year, ~ {  
  progress_bar$tick()$print()
  ensemble.downscale(., 
                     new.areas = areas, 
                     models = c("Nachman", "PL", "Poisson", 
                                "Logis", "NB"),
                     plot = FALSE,
                     verbose = FALSE)
  })

# apply_downscale_rosa_rugosa <- ensemble.downscale(
#   upgrain_rosa_rugosa, 
#   new.areas = c(10^6, 4*10^6, 9*10^6, 16*10^6), 
#   models = c("Nachman", "PL", "Poisson", "Logis", "NB", "INB"),
#   plot = TRUE,
#   verbose = TRUE)

# models = c("Nachman", "PL", "Logis", "Poisson", "NB", "GNB", "INB", "FNB")
```

We calculate the standard deviation of occupancy and tidy the data:

```{r calculate_std}
downscaling_with_std_occupancy <- map(
  names(downscaling), function(x) {
    downscaling[[x]]$Occupancy %>%
      rowwise() %>%
      mutate(std = sd(c(Nachman, PL, Poisson, Logis, NB)),
             scientificName = str_split(x, pattern = "_")[[1]][1],
             year = as.integer(str_split(x, pattern = "_")[[1]][2])) %>%
      ungroup()
  }
)
downscaling_with_std_occupancy <- bind_rows(downscaling_with_std_occupancy)

downscaling_with_std_occupancy <- crossing(
  data.frame(scientificName = species, 
             stringsAsFactors = FALSE),
  year = seq(min(downscaling_with_std_occupancy$year, na.rm = TRUE),
             max(downscaling_with_std_occupancy$year, na.rm = TRUE))) %>%
  left_join(data.frame(downscaling_with_std_occupancy, 
                       stringsAsFactors = FALSE),
            by = c("scientificName", "year"))
```
...
```{r save_occupancy}
write_tsv(downscaling_with_std_occupancy,
          path = "../data/output/downscaling_occupancy.tsv")
```

And plot them for each taxa:

```{r plot_occupancy}
occupancy_plots <- map(
  species, function(species_name)
    ggplot(downscaling_with_std_occupancy %>%

...
```{r}
taxa_output_file <- "../data/output/cheklist_taxa.tsv"
```
...
Create df of unique `nubKey`s (= the corresponding taxa in the GBIF Backbone Taxonomy, if found). We will use this list to retrieve information from GBIF (rather than doing it for each taxon, where some `nubKey`s will be repeated.
...
```{r}
checklist_taxa_with_backbone %<>%
  rename("checklist_taxonKey" = "key",
         ...
         "backbone_status" = "taxonomicStatus",
...
```{r}
write.table(checklist_taxa_with_backbone, file = taxa_output_file, sep = "\t", row.names = FALSE, quote = FALSE)
```

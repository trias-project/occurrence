...
```{r}
taxa_output_file <- "../data/output/taxa.tsv"
```

## Select checklist datasets

These are alien species checklists we want to use and combine:

```{r}
checklist_datasets <- c(
  "9ff7d317-609b-4c08-bd86-3bc404b77c42", # Manual of the Alien Plants of Belgium
  "87e99777-7f54-4209-9688-cbf7a9179ebe"  # Checklist of Danish Vascular Plants (Tracheophyta)
                                          # Just a test checklist to have some overlap in taxa
)
```
...
Create vector of unique `nubKey`s (= the corresponding taxa in the GBIF Backbone Taxonomy, if found). We will use this list to retrieve information from GBIF (rather than doing it for each taxon, where some `nubKey`s will be repeated.
...
```{r get_backbone_taxa}
backbone_taxa <- map_dfr(nub_keys, function(x) name_usage(key = x)$data) %>%
  select(taxon_properties)
```
...
```{r}
checklist_taxa_with_backbone <- left_join(checklist_taxa, backbone_taxa, by = "nubKey")
```
...
```{r}
write.table(checklist_taxa_with_backbone, file = taxa_output_file, row.names = FALSE, quote = FALSE)
```

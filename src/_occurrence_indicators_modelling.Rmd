...
Retrieve canonical names which will be useful to better discuss the results.
...
For species without canonical names (e.g. hibrids) we use scientific names instead:

```{r sc_names}
# taxa without canonical names
taxa_missing_canonicalName <- 
  spec_names %>%
  filter(is.na(canonicalName)) %>%
  distinct(taxonKey)

# retrieve scientific names for taxa without canonical names
scientificNames <- 
  map_dfr(taxa_missing_canonicalName$taxonKey,
          function(x) {
...
spec_names <- 
  spec_names %>%
  left_join(scientificNames, by = "taxonKey") %>%
...
plot_ribbon_em <- function(df_plot, df, y_axis = "obs", ptitle = NULL){
...
apply_gam <- function(df,
  ...
  # check all eval_years are valid years in df$year
  assertthat::are_equal(all(eval_years %in% unique(df$year)), TRUE)
  ...
  # Initialization
  output_model <- df
  model <- deriv1 <- deriv2 <- plot_gam <- NULL
  emerging_status_output <-
    output_model %>%
    filter(year %in% eval_years) %>%
    select(taxonKey, year) %>%
    mutate(em_status = NA_real_,
           growth = NA_real_,
           method = method_em)
  ...
  if (use_baseline == TRUE) {
    ...
    fm <- formula(obs ~ s(year, k = maxk, m = 3, bs = "tp") + s(native_obs))
  } else {
    method_em <- "basic"
  ...
  if (nrow(df) > 3 & sum(df$obs[2:nrow(df)]) != 0) {
    ...
    if (class(result)[1] %in% c("simpleWarning", "simpleError")) {
      if (verbose) {
        print(paste0("GAM (",
                     method_em,
                     ") returned an error. Not enough points.")
        )
      }
      output_model <-
        output_model %>%
      ...
      output_model$fit <- exp(temp$fit[,1] + intercept)
      output_model$ucl <- exp(temp$fit[,1] + intercept + temp$se.fit[,1] * 1.96)
      ...
      if (any(c(nrow(em1), nrow(em2)) != length(unique(output_model$year)))) {
        print(taxon_key)
        print(c(nrow(em1),nrow(em2), length(unique(output_model$year))))
        print(em1)
        print(output_model)
      }
    ...
    if (verbose) {
      print(paste0("Too few data for applying GAM (",
                   method_em,
                   ") to ", name, "(", taxon_key, ")", ". Decision rules are used."))
    }
  }
...
Apply GAM to taxa in Belgium:

```{r apply_gam_to_all_taxa, message=FALSE}
taxon_keys <- spec_names$taxonKey[1:30]
taxon_names <- spec_names$canonicalName[1:30]
gam_occs_BE <- map2(
...
taxon_keys <- spec_names$taxonKey[1]
taxon_names <- spec_names$canonicalName[1]
gam_occs_pa <- map2(
  taxon_keys, taxon_names,
  function(t, n) {
    # pb$tick()

...
```{r define_functions_GAM, code_folding ='hide'}
...
apply_gam <- function(df,
  ...
  assertthat::assert_that(
    if (isTRUE(use_baseline)) !is.null(baseline_var), 
    msg = paste0("If 'baseline' TRUE, ",
                "you need to specify which column contains baseline data.")
  )
  ...
  if (use_baseline == TRUE) {
    ...
    if (type_indicator == "observations") {
      fm <- paste0(y_var,
                 " ~ s(", 
                 year, 
                 ", k = maxk, m = 3, bs = \"tp\") + s(",

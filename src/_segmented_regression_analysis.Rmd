...
```{r define functions}
# segm_regr = apply a simple regression and try adding 1 or 2 
# break points.
# df_r = data frame with time series data
# 
segm_regr <- function(df_r){    
  cat(spec, " - ", i, " - ", max(df_r$year), "\n")  
...
plot_sr <- function(msm, df_r, minyear, maxyear, maxaantal, spec){
  ...
  for (i in 1:(nrow(msm) - 1)) {
    ...
    if (sls[i] == TRUE){mycol = "red"}else{mycol = "green"}
    ...
    coord_cartesian(xlim = c(minyear, maxyear), ylim = c(0, maxaantal)) +
    ggtitle(spec)

  ggsave(filename = paste0(spec, "_", max(df_r$year), ".png"), plot = g)
...
```{r load data}
filename <- "../data/interim/segmented_regression_test_2018_12_06.RDS"
df <- readRDS(filename)
```
...
for (spec in speclist){
  ...
  for (i in 3:nrow(df_s)){
    ...
    sls <- sign(slope(lm_s1)[[1]][,4]) == sign(slope(lm_s1)[[1]][,5])
    df[df$year == max(df_r$year) & df$species == spec, "em"] <- sls[length(sls)]
...
```{r plot_results}
group.colors <- c("TRUE" = "red", "FALSE" = "green", "unkown" = "black")
